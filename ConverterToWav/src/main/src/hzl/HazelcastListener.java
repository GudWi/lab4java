...
public class HazelcastListener implements EntryAddedListener<Long, AudioModel> {
    ...
    public void init(){
        hazelcastInstance.getMap("aiff").addEntryListener(this,true);
        idGenerator = hazelcastInstance.getFlakeIdGenerator("convertedToWav");
    }

    @Override
    public void entryAdded(EntryEvent<Long, AudioModel> entryEvent) {
        AudioModel model = entryEvent.getValue();
        File convertedFile = converter.ConvertFileToWAVE(model.getAudio());

        if (convertedFile != null){
            IMap<Object, Object> map = hazelcastInstance.getMap("convertedToWav");
